                           ...
                           double z = 0.99)
{
    ...
    double prob_bad_model = 1 - prob_all_inlier;
    double num_iters_double = std::log(1.0 - z) / std::log(prob_bad_model);
    int num_iters = static_cast<int>(math::round(num_iters_double));
    return num_iters;
...
std::vector<int> find_inliers(sfm::Correspondences2D2D const &matches, FundamentalMatrix const &F, const double &thresh)
{
    ...
    {
        double sampson_dis = calc_sampson_distance(F, matches[i]);
        if (sampson_dis < squared_thresh)
        {
            inliers.push_back(i);
        }
    }
