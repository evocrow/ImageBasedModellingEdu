              ...
              double* point_x_ptr, double* point_y_ptr)
{
    ...
    // 相机平移向量的偏导数
    cam_x_ptr[3] = f * (x * 2 * (k0 + 2*k1*r2) * x / zc + d / zc);
    cam_x_ptr[4] = f * (x * 2 * (k0 + 2*k1*r2) * y / zc);
    cam_x_ptr[5] = f * (- x * 2 * (k0 + 2*k1*r2) * r2 / zc - d * x / zc);
    cam_y_ptr[3] = f * (y * 2 * (k0 + 2*k1*r2) * x / zc);
    cam_y_ptr[4] = f * (y * 2 * (k0 + 2*k1*r2) * y / zc + d / zc);
    cam_y_ptr[5] = f * (- y * 2 * (k0 + 2*k1*r2) * r2 / zc - d * y / zc);

    // 相机旋转矩阵的偏导数
    cam_x_ptr[6] = cam_x_ptr[4] * (t[2] - zc) + cam_x_ptr[5] * (yc - t[1]);
    cam_x_ptr[7] = cam_x_ptr[3] * (zc - t[2]) + cam_x_ptr[5] * (t[0] - xc);
    cam_x_ptr[8] = cam_x_ptr[3] * (t[1] - yc) + cam_x_ptr[4] * (xc - t[0]);
    cam_y_ptr[6] = cam_y_ptr[4] * (t[2] - zc) + cam_y_ptr[5] * (yc - t[1]);
    cam_y_ptr[7] = cam_y_ptr[3] * (zc - t[2]) + cam_y_ptr[5] * (t[0] - xc);
    cam_y_ptr[8] = cam_y_ptr[3] * (t[1] - yc) + cam_y_ptr[4] * (xc - t[0]);
